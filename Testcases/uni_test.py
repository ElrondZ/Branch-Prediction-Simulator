...
TRans = "trace.txt.ans"
makefile = "Makefile"
codeName = "branchprediction.cpp"
programName = "branchprediction"
testNum = 3

def testCase(curDir, programName):
	if platform == 'win32':
		programName = 'branchprediction.exe'
	else:
		programName = 'branchprediction'
	os.chdir(curDir)
	...
	## Run program
	try:
		run_process = subprocess.Popen(["./" + programName, "config.txt", "trace"], stderr=subprocess.STDOUT)
		time.sleep(5)
	...
	if(len(f1_outputs) < 2):
		outputType = 1
		f1_outputs = f1_outputs[0]
	f2_outputs = f2.readline()
	numOfPoints = len(f2_outputs)
	crtNum  = 0
	...
	for i in range(numOfPoints ):
		if i >= outputNum:
			break
		if outputType == 0:
			if f1_outputs[i][0] == f2_outputs[i]:
				crtNum += 1
		else:
	...
	f1.close()
	os.remove(TRres)
	return 100 * crtNum / float(numOfPoints)
